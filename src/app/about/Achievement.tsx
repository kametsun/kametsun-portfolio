...
export const Achievement = (): ReactElement => {
  ...
  return (
        <div className="my-12 w-9/10 bg-background bg-opacity-70 p-4 md:p-10 md:pl-10 lg:w-3/5 xl:w-1/2">
            ...
            <table className="my-5 w-full table-auto">
                ...
                {achievements.map((achievement, index) => (
                    <tr key={index} className="hover:text-background. border-l border-emerald-400 bg-background">
                        <td className="flex h-fit p-2 md:px-4 md:py-2"><p>{achievement.date}</p>
                            <p className="ml-auto">▷</p></td>
                        <td className="border-l border-emerald-400 p-2 md:px-4 md:py-2"
                            style={{ whiteSpace: 'nowrap' }}>{achievement.content}</td>
                    </tr>
                ))}
                </tbody>
            </table>
